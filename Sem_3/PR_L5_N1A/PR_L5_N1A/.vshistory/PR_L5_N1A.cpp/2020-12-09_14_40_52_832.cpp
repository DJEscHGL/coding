...
int main() {
	...
	while (flag) {
		...
		case 2:
			...
			for (int i = 0; i < num; i++) {
				cout << transport[i]->getID() << "\t" << transport[i]->costOfKG * weight + distance * transport[i]->costOfKM << "\t" << transport[i]->costOfKG * weight << "\t" << distance * transport[i]->costOfKM << "\t" << distance / transport[i]->getMaxSpeed << endl;
			}
			system("pause");
			break;
		}
