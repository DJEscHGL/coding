...
int main() {
	...
	while (flag) {
		...
		case 2:
			system("cls");
			cout << "ID" << "\t" << "Полная" << "\t" << "Грузы" << "\t" << "Пассажиры" << "\t" << "Время перевозки" << endl;
			for (int i = 0; i < num; i++) {
				cout << transport[i]->getID() << "\t" << transport[i]->getCostKG() * weight + distance * transport[i]->getCostKM() << "\t" << transport[i]->getCostKG() * weight << "\t" << distance * transport[i]->getCostKM() << "\t\t" << distance / transport[i]->getMaxSpeed() << endl;
			}
			system("pause");
			break;
		}
